...
alias gdm='git diff --name-only $(git merge-base HEAD $(gdb))'
alias gdb='basename $(git rev-parse --abbrev-ref origin/HEAD)'
alias gl='git log --all --graph --decorate --oneline --simplify-by-decoration'
alias gpl='git pull origin $(git branch --show-current)'
alias gplm='echo git fetch origin && echo git pull origin $(gdb)'
